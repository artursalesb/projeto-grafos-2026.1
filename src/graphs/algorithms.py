"""
Algoritmos de grafos implementados do zero (sem networkx/scipy/etc.).

Funções para Parte 1 (grafo não-direcionado, pesos >= 0):
  bfs, dfs, dijkstra, bellman_ford

Funções extras para Parte 2 (grafo dirigido, pesos arbitrários):
  bfs_levels         → camadas / ordem de visita
  dfs_classify       → classificação de arestas + detecção de ciclo
  bellman_ford_full  → BF correto p/ dirigido + ciclo negativo

As 4 funções 'clássicas' mantêm assinatura compatível com solve.py da Parte 1.
"""
import heapq
